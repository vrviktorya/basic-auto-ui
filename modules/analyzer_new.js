...
async function analyzeDesignSystem(url) {
    ...
    try {
        ...
        return {
            ...
            designSystem: buildDesignSystemJSON({
                url,
                domain: new URL(url).hostname,
                colors: colorAnalysis,
                typography: {
...
function buildDesignSystemJSON(result) {
    return {
        url: result.url,
        domain: result.domain,
        colors: {
            palette: result.colors?.palette || [],
            total: result.colors?.total || 0,
            semantics: result.colors?.semantics || {},
            analysisVersion: result.colors?.analysisVersion || 'unknown'
        },
        typography: {
            total: result.typography?.total || 0,
            styles: result.typography?.styles || []
        },
        buttons: {
            total: result.buttons?.total || 0,
            found: result.buttons?.found || false,
            clusters: result.buttons?.clusters || {},
            samples: result.buttons?.samples || []
        }
    };
}
